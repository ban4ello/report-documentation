...
const login = async ({ email, password }) => {
  ...
  try {
    ...
    console.log(error);
    showNotif({ message: error.response.data.message, type: 'error' });
...
const signup = async () => {
  try {
    loading.value = true;
    const signupResponse = await ApiService.signup(newUserData.value);

    login({ email: newUserData.value.email, password: newUserData.value.password });
  } catch (error) {
    console.log(error);
    showNotif({ title: error.response.data.message, type: 'error' });
  } finally {
    loading.value = false;
  }
};
  ...
  <div class="bg-surface-50 dark:bg-surface-950 flex items-center justify-center min-h-screen min-w-[100vw] overflow-hidden">
      ...
      <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
        <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
          ...
          <div>
            ...
            <InputText v-model="userData.email" required id="email1" type="text" placeholder="example@gmail.com" class="w-full md:w-[30rem] mb-8" />
    ...
    <Dialog v-model:visible="visible" pt:root:class=" !bg-transparent" pt:mask:class="backdrop-blur-sm" class="w-[450px]">
      <template #container="{ closeCallback }">
        <div class="flex justify-center items-center flex-col px-8 py-8 gap-6 rounded-2xl" style="background-image: radial-gradient(circle at left top, var(--p-primary-400), var(--p-primary-700))">
          <div class="w-[80px] mb-4">
            <svg fill="#000000" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 496 496" xml:space="preserve">
              <g>
                <g>
            ...
            l26.672-16H384V480z M416,480h-16v-32h16V480z M464,480h-32v-32h32c8.816,0,16,7.176,16,16C480,472.824,472.816,480,464,480z"
                    />
                    <path fill="#fff" d="M304,48V0H0v416h304v-48h192V48H304z M288,400H16V128h272V400z M288,112H16V16h272V112z M480,352H304V64h176V352z" />
                    <path
                      fill="#fff"
                      d="M387.704,334.752l4.296,2.736l4.296-2.736C399.064,332.984,464,290.648,464,208S399.064,83.016,396.296,81.248
            L392,78.512l-4.296,2.736C384.936,83.016,320,125.352,320,208S384.936,332.984,387.704,334.752z M392.016,97.848
            ...
            c-8.776-6.728-29-24.344-42.584-52.24C361.344,274.72,376.048,280,392,280z"
                    />
                    <polygon fill="#fff" points="384,248 400,248 400,216 432,216 432,200 400,200 400,168 384,168 384,200 352,200 352,216 384,216 			" />
                  </g>
                </g>
              </g>
            </svg>
          ...
          <div class="inline-flex flex-col gap-2">
            ...
            <InputText v-model="newUserData.password" aria-label="password" :invalid="newUserData.password !== newUserData.сonfirmPassword" id="password" class="!bg-white/20 !p-4 !text-primary-50 w-80" type="password"></InputText>
            <Message v-if="newUserData.password !== newUserData.сonfirmPassword" severity="error" size="small" variant="simple">Пароли не соападают</Message>
          </div>

          <div class="inline-flex flex-col gap-2">
            <label for="сonfirmPassword" class="text-primary-50 font-semibold">Повторите пароль</label>
            <InputText v-model="newUserData.сonfirmPassword" :invalid="newUserData.сonfirmPassword !== newUserData.password" id="сonfirmPassword" class="!bg-white/20 !p-4 !text-primary-50 w-80" type="password"></InputText>
            <Message v-if="newUserData.сonfirmPassword !== newUserData.password" severity="error" size="small" variant="simple">Пароли не соападают</Message>
          </div>
          ...
          <div class="flex items-center justify-between gap-4 w-full">
            ...
            <Button type="submit" :disabled="newUserData.password && newUserData.сonfirmPassword && newUserData.password !== newUserData.сonfirmPassword && newUserData.email" label="Зарегистрироваться" @click="signup" text class="!p-4 w-full !text-primary-50 !border !border-white/30 hover:!bg-white/10"></Button>
          </div>
        </div>
      </template>
  </Dialog>
