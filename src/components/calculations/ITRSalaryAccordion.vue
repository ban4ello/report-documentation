...
// Фильтрованный список имен ИТР
const filteredITRNames = computed(() => {
  ...
  if (isObjectFormat) {
    ...
    if (isShowOnlyType.value) {
      // Показываем только ИТР (position === 'itr')
      return props.dropdownItemsWorkerStaff.filter((worker) => worker.position === 'itr').map((worker) => worker.name);
    } else {
      // Показываем всех
            ...
            >
              ...
              <Column field="name" header="Имя сотрудника" style="width: 25%">
                ...
                <template #editor="{ data }">
                  <Select id="staff" v-model="data.name" :options="filteredITRNames" class="w-full"></Select>
                </template>
              </Column>
